...
describes what users get. Pick a format with `-o`, or let dailymed choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dailymed <command> -o table   # aligned columns for reading
dailymed <command> -o jsonl   # one JSON object per line, for piping
dailymed <command> -o json    # a single JSON array
dailymed <command> -o csv     # spreadsheet friendly
dailymed <command> -o tsv     # tab-separated
dailymed <command> -o url     # just the URL column
dailymed <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
dailymed <command> --fields id,title,url
```
...
```bash
dailymed <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
dailymed <command>            # a table, because this is a terminal
dailymed <command> | wc -l    # JSONL, because this is a pipe
```
