...
// Domain also builds the standalone dailymed binary (see cli.NewApp), so the
...
func (Domain) Register(app *kit.App) {
	...
	// Resolver op: one record per id, the home of `dailymed page` and
	// `ant get dailymed://page/<id>`.
	kit.Handle(app, kit.OpMeta{Name: "page", Group: "read", Single: true,
		Summary: "Fetch a page by path or URL", URIType: "page", Resolver: true,
		Args: []kit.Arg{{Name: "ref", Help: "page path or URL"}}}, getPage)

	// List op: members of a page, the home of `dailymed links` and `ant ls`.
	// It emits page stubs, so every listed member is itself an addressable
	// dailymed://page/ URI a host can follow.
...
// Classify turns any accepted input — a bare path or a full dailymed.nlm.nih.gov URL —
